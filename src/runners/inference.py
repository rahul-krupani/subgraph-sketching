...
@torch.no_grad()
def test(model, evaluator, train_loader, val_loader, test_loader, args, device, num_nodes, emb=None, eval_metric='hits'):
    print('starting testing')
    t0 = time.time()
    model.eval()
    print("get train predictions")
    test_func = get_test_func(args.model)
    pos_train_pred, neg_train_pred, train_pred, train_true = test_func(model, train_loader, device, args, num_nodes, split='train')
    print("get val predictions")
    pos_val_pred, neg_val_pred, val_pred, val_true = test_func(model, val_loader, device, args, num_nodes, split='val')
    print("get test predictions")
    pos_test_pred, neg_test_pred, test_pred, test_true = test_func(model, test_loader, device, args, num_nodes, split='test')
...
@torch.no_grad()
def get_buddy_preds(model, loader, device, args, num_nodes, split=None):
    n_samples = get_split_samples(split, args, len(loader.dataset))
    t0 = time.time()
    preds = []
    data = loader.dataset
    ...
    for batch_count, indices in enumerate(tqdm(loader)):
        ...
        if args.use_RA:
            ...
            RA = None
        logits = model(subgraph_features, node_features, curr_links, num_nodes, degrees[:, 0], degrees[:, 1], RA, batch_emb)
