...
def run(args):
    ...
    for rep in range(args.reps):
        set_seed(rep)
        dataset, splits, directed, eval_metric, num_nodes = get_data(args)
        ...
        for epoch in range(args.epochs):
            t0 = time.time()
            loss = train_func(model, optimizer, train_loader, args, device, num_nodes)
            if (epoch + 1) % args.eval_steps == 0:
                results = test(model, evaluator, train_eval_loader, val_loader, test_loader, args, device, num_nodes,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default='Cora',
                        choices=['Cora', 'Citeseer', 'Pubmed', 'ogbl-ppa', 'ogbl-collab', 'ogbl-ddi',
                                 'ogbl-citation2', 'ogbl-vessel', 'ogbl-biokg', 'ogbl-wikikg2'])
    parser.add_argument('--val_pct', type=float, default=0.03,
    ...
    parser.add_argument('--test_pct', type=float, default=0.03,
    ...
    # if args.dataset_name == 'ogbl-ddi' or args.dataset_name == 'ogbl-biokg':
