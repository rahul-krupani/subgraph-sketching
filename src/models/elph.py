...
class BUDDY(torch.nn.Module):
    ...
    def __init__(self, args, num_features=None, node_embedding=None):
        ...
        if self.sign_k != 0:
            if self.propagate_embeddings:
                ...
                #GATConv(4*args.hidden_channels, args.hidden_channels, heads=1, dropout=args.sign_dropout)
                GATConv(1,args.hidden_channels, heads=1, dropout=args.sign_dropout)
                for _ in range(args.sign_k)
            ])
    ...
    def feature_forward(self, x, curr_links, num_nodes):
        """
        small neural network applied edgewise to hadamard product of node features
        @param x: node features torch tensor [batch_size, 2, hidden_dim]
        @return: torch tensor [batch_size, hidden_dim]
        ...
        if self.sign_k != 0:
            ...
            #    print("Current shape: ", x.shape)
                ...
                x = gat_layer(torch.ones((num_nodes, 1)), curr_links.t())[curr_links]
    ...
    def forward(self, sf, node_features, curr_links, num_nodes, src_degree=None, dst_degree=None, RA=None, emb=None):
        """
        ...
        @return:
        """
        #print("Subgraph features SF: ", sf.shape)
        if self.append_normalised:
            sf = self._append_degree_normalised(sf, src_degree, dst_degree)
        #print("Subgraph features SF: ", sf.shape)
        x = self.label_lin_layer(sf)
        #print("X after label_lin_layer: ", x.shape)
        x = self.bn_labels(x)
        #print("X after bn_labels: ", x.shape)
        x = F.relu(x)
        x = F.dropout(x, p=self.label_dropout, training=self.training)
        #print("Curr_links: ", curr_links.shape)
        if self.use_feature:
            #print("Before feature forward: ", node_features.shape)
            node_features = self.feature_forward(node_features, curr_links, num_nodes)
            #print("After feature forward: ", node_features.shape)
            x = torch.cat([x, node_features.to(torch.float)], 1)
        if self.node_embedding is not None:
            #print("Before node_embedding: ", emb.shape)
            node_embedding = self.embedding_forward(emb)
            #print("After node_embedding: ", node_embedding.shape)
            x = torch.cat([x, node_embedding.to(torch.float)], 1)
        if self.use_RA:
            RA = RA.unsqueeze(-1)
            RA = self.bn_RA(RA)
